  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.status === 0) {
        ...
        $.ajax({
          ...
          data: {
            location: city,
            key: 'd76017d24b274c168618f2c411f0886a',
            output: 'jsonp',
          },
          dataType: 'jsonp',
          success: function (data) {
            if (data.code === "200") {
              ...
              for (let i = 0; i < locations.length; i++) {
                ...
                if (location.name.includes(district.replace(/区|县/, ''))) {
                  ...
                  defaultInfo.qweather_url = `https://devapi.qweather.com/v7/weather/now?location=${location.id}&key=d76017d24b274c168618f2c411f0886a`;
                  break;
                } else {
                  console.log(locations[0].id);
                  defaultInfo.qweather_url = `https://devapi.qweather.com/v7/weather/now?location=${locations[0].id}&key=d76017d24b274c168618f2c411f0886a`;
                }
              }

              $.ajax({
                type: 'get',
                url: defaultInfo.qweather_url,
                data: {
                  output: 'jsonp',
                },
                dataType: 'jsonp',
    ...
    error: function (error) {
      ...
      if (document.getElementById('hexo_electric_clock')) {
        $.ajax({
          type: 'get',
          url: `https://devapi.qweather.com/v7/weather/now?location=${clock_rectangle}&key=${qweather_key}`,
          data: {
            output: 'jsonp',
          },
          dataType: 'jsonp',
